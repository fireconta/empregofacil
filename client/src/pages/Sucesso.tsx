...
import { Download, Share2, CheckCircle2, AlertCircle, Lock } from "lucide-react";
import Layout from "@/components/Layout";
import { APP_DOWNLOAD_LINK, SHARE_MESSAGE } from "@/config";
...
export default function Sucesso() {
  ...
  const isCompleted = downloadClicked && isShared;
        ...
        <div className="max-w-3xl w-full">
          ...
          <div className="bg-card border border-border shadow-xl rounded-2xl overflow-hidden">
            ...
            <div className="p-8 md:p-12 space-y-8">
              ...
              <div className={`relative flex flex-col md:flex-row gap-6 items-start p-6 rounded-xl border-2 transition-all ${!isShared ? "opacity-50 grayscale border-muted" : downloadClicked ? "border-green-500/20 bg-green-50/50" : "border-primary/10 bg-muted/20"}`}>
                ...
                <div className={`h-12 w-12 rounded-full flex items-center justify-center shrink-0 text-lg font-bold ${downloadClicked ? "bg-green-500 text-white" : "bg-primary text-white"}`}>
                  {downloadClicked ? <CheckCircle2 className="h-6 w-6" /> : "2"}
                </div>
                <div className="flex-1 space-y-4">
                  <div>
                    <h3 className="text-xl font-bold text-foreground mb-1">Instalação Completa do App</h3>
                    <p className="text-muted-foreground">
                      Instale o aplicativo oficial e abra-o para confirmar seu dispositivo. <strong>Seu currículo só será enviado após a instalação completa e correta.</strong>
                    </p>
                  </div>
                  <Button 
                    ...
                    disabled={!isShared}
                    size="lg" 
                    className={`w-full md:w-auto ${downloadClicked ? "bg-green-600 hover:bg-green-700" : ""}`}
                  >
                    <Download className="mr-2 h-5 w-5" />
                    {downloadClicked ? "Instalando..." : "Baixar e Instalar App"}
                  </Button>
                </div>
              </div>
              ...
              <div className="pt-4 text-center">
                {isCompleted ? (
                  <div className="animate-in zoom-in duration-500 p-6 bg-green-50 rounded-xl border border-green-200">
                    <h3 className="text-2xl font-bold text-green-700 mb-2">Aguardando Instalação... ⏳</h3>
                    <p className="text-green-800 font-medium">
                      Detectamos o clique no download. Assim que a instalação for concluída no seu celular, seu currículo será enviado automaticamente.
                    </p>
                    <p className="text-sm text-green-700 mt-2">
                      Acompanhe o status da sua candidatura diretamente pelo aplicativo.
                    </p>
                  </div>
                ) : (
